...
class BaseChatModel(ABC):
    ...
    def chat(self, request: ChatRequest) -> ChatResponse:
        """
        Send a chat request to the provider and return a normalised ChatResponse.
        """
        raise NotImplementedError

    def get_model_name(self, request: ChatRequest) -> str:
        model_name = request.model or self.default_model
        if not model_name:
            raise ConfigurationError(
                f"No model configured for provider {self.provider_name!r}."
            )
        return model_name
    ...
    def prepare_request(self, request: ChatRequest) -> ChatRequest:

...
class AnthropicChatPricing(BaseModel):
...
class AnthropicChatUsageDetails(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AnthropicChatCostDetails(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
def list_pricings() -> list[AnthropicChatPricing]:
    return load_pricing_catalogue(PRICING_PATH, AnthropicChatPricing)
...
def get_pricing(
    ...
    return select_pricing(
        list_pricings(), model, provider_name="anthropic", effective_at=effective_at
    )
...
def calculate_cost_summary(
    *,
    usage: AnthropicChatUsageDetails,
    pricing: AnthropicChatPricing,
...
def calculate_cost_details(
    *,
    ...
    return AnthropicChatCostDetails(
        ordinary_input_cost=breakdown.input_cost,
        cache_read_cost=breakdown.cached_read_cost,

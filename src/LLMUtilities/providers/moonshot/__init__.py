...
from .pricing import (
    MoonshotChatCostDetails,
    MoonshotChatPricing,
    MoonshotChatUsageDetails,
...
class MoonshotProvider:
        ...
        *,
        ...
        if not _chat_completions_available():
            raise MissingDependencyError(
                "The 'openai' package is required for the Moonshot provider "
                "(Moonshot speaks the OpenAI-compatible Chat Completions "
        ...
        self._defaults = settings.moonshot
    ...
    def list_pricings(self) -> list[MoonshotChatPricing]:
        return list_pricings()

    def get_pricing(self, model: str, *, effective_at=None) -> MoonshotChatPricing:
        return get_pricing(model, effective_at=effective_at)

    def chat(self, request: ChatRequest) -> ChatResponse:
        model_name = _chat.resolve_model_name(
            request, self._defaults.chat_model, "moonshot"
        )
    ...
    def get_detailed_usage(self, response: ChatResponse) -> MoonshotChatUsageDetails:
        input_tokens, output_tokens, total_tokens, cached_input_tokens = (
            _chat.extract_usage_totals(response.raw)
        )
        return MoonshotChatUsageDetails(
    ...
    def get_detailed_cost(self, response: ChatResponse) -> MoonshotChatCostDetails:
    ...
    def _resolve_pricing(self, response: ChatResponse) -> MoonshotChatPricing:
    ...
    def _require_api_key(self) -> str:
        if not self.api_key:
            raise ConfigurationError("Missing API key for provider 'moonshot'.")
        return self.api_key

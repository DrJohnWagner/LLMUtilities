...
def _get_env_positive_float(name: str, default: float) -> float:
    ...
    if value is None or value <= 0:
        raise ValueError(
            f"Environment variable {name} must be greater than zero, got: {value!r}"
        )
...
def _get_env_non_negative_int(name: str, default: int) -> int:
    ...
    if value is None or value < 0:
        raise ValueError(
            f"Environment variable {name} must be zero or greater, got: {value!r}"
        )
...
def _get_default_provider() -> str:
    provider = (
        _get_env_str("LLMUTILITIES_DEFAULT_PROVIDER", "openai") or "openai"
    ).lower()
...
def _resolve_google_api_key() -> Optional[str]:
    """``GOOGLE_API_KEY`` takes precedence; ``GEMINI_API_KEY`` is the fallback alias."""
    return _get_env_str("GOOGLE_API_KEY") or _get_env_str("GEMINI_API_KEY")
...
def get_settings() -> Settings:
    return Settings(
        ...
        openai=ProviderDefaults(
            chat_model=_get_env_str("OPENAI_CHAT_MODEL", DEFAULT_MODELS["openai"]),
        ...
        anthropic=ProviderDefaults(
            chat_model=_get_env_str(
                "ANTHROPIC_CHAT_MODEL", DEFAULT_MODELS["anthropic"]
            ),
            temperature=_get_env_float("ANTHROPIC_TEMPERATURE"),
            max_output_tokens=_get_env_int("ANTHROPIC_MAX_OUTPUT_TOKENS"),
        ),
        google=ProviderDefaults(
            chat_model=_get_env_str("GOOGLE_CHAT_MODEL", DEFAULT_MODELS["google"]),
            embedding_model=_get_env_str(
        ...
        moonshot=ProviderDefaults(
            chat_model=_get_env_str(
                "MOONSHOT_CHAT_MODEL", DEFAULT_MODELS["moonshot"]
            ),
        ...
        deepseek=ProviderDefaults(
            chat_model=_get_env_str(
                "DEEPSEEK_CHAT_MODEL", DEFAULT_MODELS["deepseek"]
            ),
        ...
        max_retries=_get_env_non_negative_int("LLMUTILITIES_MAX_RETRIES", 3),
    )
